...
- Granted official tools also travel through this signed HTTP bridge. From the app backend, use `list_official_tools`, `get_official_tool`, `call_official_tool`, or the Chrome-specific helpers from `commons/backend/forger_desktop.py`. Do not call Desktop official tool endpoints from frontend code.
