...
- `tools.required` means the app declares a functional dependency, but Desktop still allows installation when the tool is missing or unconfigured. At runtime, calls to missing, inactive, unconfigured, or undeclared tool actions fail with a tool access error.
- `tools.optional` starts enabled in the install or review access modal when there is no saved grant. The person can disable it before confirming, and can connect or disconnect it later from the installed app detail view.
...
- `platformCapabilities` are informative runtime declarations shown in Forger's access review and app detail views. They are not user grants, official tools, app secrets, scripts, catalog capabilities, or provider filesystem permissions.
