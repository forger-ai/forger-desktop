...
description: Use when operating Forger Tools, Connections, installed app lifecycle/app-view actions, app prompt actions, Forger Chrome Extension actions, or platform actions; especially when deciding between Forger app windows, runtime diagnostics, and external Chrome browser control without non-Forger fallbacks.
---
...
- Use `forger_create_app` only after you have clarified the app direction. The tool creates the skeleton-based app workspace; continue design and implementation in the same chat after it succeeds. If you apply changes in the created app during that turn, finish with the app saved as an internal version so the next turn starts from a clean saved state.
...
- For installed apps, "open", "launch", "start", "run", or "bring up" means use the Forger app tools. Use `forger_open_app` to open the app, `forger_restart_app` to recover stale runtime state, and `forger_get_app_runtime_status` when you need to check whether Forger has it running.
...
- For installed app debugging, inspect the Desktop-owned app window with `forger_get_app_view_snapshot` and runtime/log context with `forger_get_app_runtime_diagnostics` when available. These helpers take an `appId`, not a raw runtime URL.
