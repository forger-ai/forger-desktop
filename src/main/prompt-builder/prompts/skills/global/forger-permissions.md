...
- Folder grants are Forger-owned filesystem permissions. They are separate from `permissionMode`, Forger Tools, Connections, app secrets, scripts, MCP tools, and app catalog copy.
...
- Elevated permissions do not bypass Forger MCP approvals, Connection grants, secret handling, destructive-action confirmations, or tool allowlists.
...
- When a Forger Tool or Connection returns a permission denial, cancellation, or unavailable approval result, do not retry through another integration.
