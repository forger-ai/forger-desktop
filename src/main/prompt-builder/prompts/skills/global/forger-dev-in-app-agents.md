...
- Granted Forger Tools and Connections also travel through this signed HTTP bridge. From the app backend, use `list_official_tools`, `get_official_tool`, `call_official_tool`, or the Chrome-specific helpers for Forger Tools; use `list_connections`, `connection_status`, `configure_connection`, and `call_connection_action` for registered Connections such as Gmail, Google Calendar, Google Sheets, Google Drive, Google Docs, GitHub, Notion, WhatsApp, Slack, and Trello. Optional Connection access is granted from the manifest review surfaces during install/download or later from Forger's app access UI; app code must handle missing optional grants as a normal unavailable state. Do not collect OAuth secrets in the app and do not call Desktop bridge endpoints from frontend code.
- For Forger Chrome Extension helpers, use `chrome_set_styles` and `chrome_highlight_element` only for temporary visual highlighting or restoring selected elements with allowed CSS properties. Treat `chrome_submit_form` as sensitive because it can send data or trigger remote changes.
...
- Pass `runtime={ "provider": "...", "model": "...", "effort": "..." }` to `start_agent_task` only when the app manifest declares `platformCapabilities.agentRuntimeControl`. Without that capability, task runs use the runtime configured by the prompt template, prompt override, or Desktop defaults.
...
- Pass `runtime` to `start_manifest_agent_thread`, `resume_manifest_agent_thread`, or `steer_manifest_agent_run` only when the manifest declares `platformCapabilities.agentRuntimeControl`. Without that capability, manifest agent runs use the runtime configured by the agent, prompt override, or Desktop defaults.
...
- `agentRuntimeControl` does not connect providers or install models. It only allows the app backend to choose an already-supported provider/model/effort for a specific task or agent run.
