...
- Granted Forger Tools and Connections also travel through this signed HTTP bridge. From the app backend, use `list_official_tools`, `get_official_tool`, `call_official_tool`, or the Chrome-specific helpers for Forger Tools; use `list_connections`, `connection_status`, `configure_connection`, `request_connection_grant`, and `call_connection_action` for registered Connections such as Gmail, Google Calendar, Google Sheets, Google Drive, Google Docs, GitHub, Notion, WhatsApp, Slack, and Trello. Do not collect OAuth secrets in the app and do not call Desktop bridge endpoints from frontend code.
