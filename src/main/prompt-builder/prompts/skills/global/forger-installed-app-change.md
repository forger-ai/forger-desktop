...
- Treat `frontendUrl`, `backendUrl`, localhost ports, and URLs returned by `forger_restart_app`, `forger_open_app`, or runtime status tools as internal diagnostics only. Never open, navigate to, or inspect installed app runtime URLs with Chrome Extension, a custom URL, a system browser, provider-native browser tools, `open`, `WebFetch`, or non-Forger browser control.
- For post-edit app debugging, use `forger_get_app_runtime_status`, `forger_get_app_view_snapshot`, and `forger_get_app_runtime_diagnostics`. The snapshot reads the Forger-owned app window by `appId`; runtime diagnostics provide status, app-window state, and recent logs.
