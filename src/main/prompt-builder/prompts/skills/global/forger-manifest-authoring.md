...
- Agents may edit `manifest.json` when an app needs platform runtime access. Adding `platformCapabilities`, `localNetworkShare`, `remoteTunnel`, or `tools.required` is the app declaring its runtime contract; after the app is restarted, Desktop re-reads the manifest and wires the matching platform bridge, access metadata, or required official tool actions. `tools.optional` is the exception: optional tools still need a user grant or approval before the app can use them.
...
- `actions` remains a string array. It may contain the special token `"*"`, which means every current action for that single `toolId`; it never grants actions from another tool. Use `["*"]` only when the app genuinely needs the full surface of that official tool.
- `tools.required` means the app declares a functional dependency. After the app restarts, Desktop wires the declared required action set for that app. Desktop still allows installation when the underlying tool is missing or unconfigured; at runtime, calls to missing, inactive, unconfigured, or undeclared tool actions fail with a tool access error.
- `tools.optional` requires a user grant or approval before the app can use the tool. It may be preselected in an install or review access modal when there is no saved grant, but it is not available to the app until the person confirms it. The person can connect or disconnect optional tools later from the installed app detail view.
...
- `platformCapabilities` are runtime declarations shown in Forger's access review and app detail views. They are not user grants, official tools, app secrets, scripts, catalog capabilities, or provider filesystem permissions. After the app restarts, Desktop re-reads supported platform capabilities and wires the matching signed runtime bridge behavior for that app.
...
- After changing manifest runtime wiring, including `platformCapabilities`, `tools.required`, `tools.optional`, `localNetworkShare`, `remoteTunnel`, services, app agents, prompt templates, or app secrets, restart the app before claiming the new contract is available.
