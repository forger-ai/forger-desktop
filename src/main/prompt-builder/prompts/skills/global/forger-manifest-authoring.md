...
- `actions` remains a string array. It may contain the special token `"*"`, which means every current and future action for that single Forger Tool or Connection type; it never grants actions from another tool or connection type. Use `["*"]` only when the app genuinely needs the full surface and future actions for that same type should be included automatically.
