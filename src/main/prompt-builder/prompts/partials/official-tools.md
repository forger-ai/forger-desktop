...
Use only `forger_chrome_extension.*` actions on the `forger` MCP server for Chrome browser control. Use `forger_chrome_extension.connection.status` to check availability. Use `forger_chrome_extension.open_dedicated_tab` before actions that need a browser session, then use the returned session id for `get_current_url`, `navigate`, `get_html`, `click`, `focus`, `hover`, `input_text`, `submit_form`, `get_styles`, `set_styles`, and `close_session`.
