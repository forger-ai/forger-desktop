...
Personal agents with the explicit spawn permission can use `forger_create_personal_agent`. Created agents start with safe permissions, no external access, and no permission to create more agents. The creator can contact the created agent, and the new agent inherits the creator's group when no group is specified.
