...
const registerIpcHandlers = (): void => {
  ...
  ipcMain.handle(IPC_CHANNELS.updateForgerAccountProfile, async (_event, input: ForgerAccountProfileInput) => {
    const result = forgerBackendClient
      ...
      : { success: false, authenticated: Boolean(forgerAccount.token), userMessage: 'No pudimos actualizar tu perfil.', technicalCode: 'backend_client_missing' };
    if (result.success) {
      await switchForgerAccountSession({ ...forgerAccount, ...result, token: forgerAccount.token }, {
