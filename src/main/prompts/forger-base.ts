...
export const buildGlobalForgerAgentsMarkdown = (): string => {
  return [
    ...
    '- Forger can expose official tools such as Gmail through the `forger` MCP server. Official tools are not installed apps.',
    '- Use only Forger-provided official tools for Gmail. Do not use Codex-local connectors, `codex_apps`, browser mail sessions, or personal Codex plugins inside Forger.',
    '- Gmail search, read, and send actions are sensitive and must go through the Forger MCP permission broker when approval is enabled.',
