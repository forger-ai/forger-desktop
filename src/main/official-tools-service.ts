...
const buildToolUnavailableResult = (
  ...
  if (tool.status === 'available') {
    return {
      success: false,
      userMessage: surface === 'agent'
        ? 'Gmail esta desactivada. Pidele al usuario activar y conectar Gmail en la vista Tools de Forger antes de leer o enviar correos.'
        : 'Gmail esta desactivada. Activa y conecta Gmail en la vista Tools de Forger antes de usarla desde esta app.',
  ...
  if (tool.status === 'installed') {
    return {
      success: false,
      userMessage: surface === 'agent'
        ? 'Gmail esta activada, pero todavia no esta conectada. Pidele al usuario conectar Gmail en la vista Tools de Forger.'
        : 'Gmail esta activada, pero todavia no esta conectada. Conecta Gmail en la vista Tools de Forger.',
  ...
  if (tool.status === 'error') {
    return {
      success: false,
      userMessage: `Gmail tiene un error de configuracion${tool.error ? `: ${tool.error}` : '.'}`,
      technicalCode: 'tool_configuration_error',
    };
  }
  return { success: false, userMessage: 'La herramienta no esta lista para usarse.', technicalCode: 'tool_not_configured' };
};
...
export class OfficialToolsService {
  ...
  async refresh(): Promise<OfficialToolsState> {
    return this.list();
  }

  async list(): Promise<OfficialToolsState> {
    await this.load();
    return {
      tools: await Promise.all(INTERNAL_TOOL_MODULES.map((toolModule) => this.toSummary(toolModule.definition))),
    };
  }

  async getTool(toolId: string): Promise<OfficialToolSummary | null> {
    await this.load();
    const toolModule = this.modulesById.get(toolId);
    return toolModule ? await this.toSummary(toolModule.definition) : null;
  }

  async activate(toolId: string): Promise<ToolMutationResult> {
    await this.load();
    const toolModule = this.modulesById.get(toolId);
    if (!toolModule) {
      return { success: false, userMessage: 'La herramienta no esta disponible.', technicalCode: 'tool_not_found' };
    ...
    return { success: true, userMessage: 'Herramienta activada.', tool: await this.toSummary(toolModule.definition) };
  }
  ...
  async configure(input: ConfigureOfficialToolInput): Promise<ToolMutationResult> {
    ...
    if (!this.registry.installed[input.toolId]) {
      const activation = await this.activate(input.toolId);
      if (!activation.success) {
        return activation;
      }
    }

    const result = await toolModule.configure(this.getContext());
    await this.recordError(input.toolId, result.success ? undefined : new Error(result.technicalCode ?? 'tool_configuration_failed'));
    return {
      ...result,
      tool: await this.toSummary(toolModule.definition),
    };
  }

  async deactivate(toolId: string, options?: { keepSecrets?: boolean }): Promise<ToolMutationResult> {
    await this.load();
    const toolModule = this.modulesById.get(toolId);
    if (!toolModule) {
      return { success: false, userMessage: 'La herramienta no esta disponible.', technicalCode: 'tool_not_found' };
    ...
    await this.saveRegistry();
    return { success: true, userMessage: 'Herramienta desactivada.', tool: await this.toSummary(toolModule.definition) };
  }

  async getInstallGate(appId: string): Promise<AppToolsInstallGate | null> {
    await this.load();
    const declarations = await this.options.getAppToolDeclarations(appId);
    if (!declarations) {
      return null;
    }
    const required = await Promise.all(declarations.required.map((declaration) => this.toRequirement(appId, declaration, true)));
    const optional = await Promise.all(declarations.optional.map((declaration) => this.toRequirement(appId, declaration, false)));
    return {
      appId,
  ...
  async setAppToolGrant(input: SetAppToolGrantInput): Promise<AppToolsInstallGate | null> {
    await this.load();
    ...
    await this.saveRegistry();
    return this.getInstallGate(input.appId);
  }
  ...
  private async toRequirement(appId: string, declaration: AppToolDeclaration, required: boolean) {
  ...
  private async toSummary(entry: OfficialToolDefinition): Promise<OfficialToolSummary> {
    const installed = this.registry.installed[entry.id];
    const configured = await this.isConfigured(entry);
    if (!installed) {
      return { ...entry, status: 'available', configured: false };
    }
    return {
      ...entry,
