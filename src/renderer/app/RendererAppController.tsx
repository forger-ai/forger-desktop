...
const initialSettings: Settings = { userEmail: '', plan: 'Free', safeMode: false, developerMode: { enabled: false, pathEntries: [] }, codexDefaults: { model: 'gpt-5.4', reasoningEffort: 'medium', }, defaultAgentProvider: 'auto', defaultChatPermissionMode: 'safe', agentDefaults: { codex: { model: 'gpt-5.4', reasoningEffort: 'medium', }, claude: { model: 'claude-sonnet-4-6', effort: 'high', }, }, providerConnections: {}, };
...
if (settingsResult.status === 'fulfilled') { setSettings(settingsResult.value); setSelectedChatPermissionMode(settingsResult.value.defaultChatPermissionMode ?? 'safe'); }
...
return { ...conversation, threadId: typeof run.threadId === 'string' && run.threadId.trim().length > 0 ? run.threadId : conversation.threadId, updatedAt: new Date().toISOString(), messages: [ ...conversation.messages, { id: messageId, role: 'assistant', content: assistantContent, ...(questionAction ? { action: questionAction } : {}), }, ], };
}), ); traceChatEvent({ event: 'chat_run_message_appended', runId: run.runId, appId: run.appId, conversationId: targetConversationId, activeConversationId: activeConversationIdRef.current, status: run.status, foundConversation }); if (run.createdApp) { void refreshApps().then(() => startCreatedAppConversation(run.createdApp as Parameters<typeof startCreatedAppConversation>[0])); }
...
const handleAgentDefaultsChange = async (input: UpdateAgentDefaultsInput) => { setSettings((current) => { const nextDefaultProvider = input.defaultProvider ?? current.defaultAgentProvider; const nextDefaultChatPermissionMode = input.defaultChatPermissionMode ?? current.defaultChatPermissionMode; if (!input.provider) { return { ...current, defaultAgentProvider: nextDefaultProvider, defaultChatPermissionMode: nextDefaultChatPermissionMode }; }
...
return { ...current, defaultAgentProvider: nextDefaultProvider, defaultChatPermissionMode: nextDefaultChatPermissionMode, codexDefaults: nextCodexDefaults, agentDefaults: { ...current.agentDefaults, codex: nextCodexDefaults, }, };
}
return { ...current, defaultAgentProvider: nextDefaultProvider, defaultChatPermissionMode: nextDefaultChatPermissionMode, agentDefaults: { ...current.agentDefaults, claude: { model: input.model ?? current.agentDefaults.claude.model, effort: (input.effort as ClaudeEffort | undefined) ?? current.agentDefaults.claude.effort, }, }, };
}); try { const updated = await getDesktopApi().updateAgentDefaults(input); setSettings(updated); setSelectedChatPermissionMode(updated.defaultChatPermissionMode ?? 'safe'); setSelectedCodexModel(updated.agentDefaults.codex.model); setSelectedCodexReasoningEffort(updated.agentDefaults.codex.reasoningEffort); setSelectedClaudeModel(updated.agentDefaults.claude.model); setSelectedClaudeEffort(updated.agentDefaults.claude.effort); if (selectedAppDetailsId) { setSelectedAppDetails(await getDesktopApi().getAppDetails(selectedAppDetailsId)); }
...
const handleRespondQuestion = async ( runId: string, request: ChatQuestionRequest, response: ChatQuestionResponse, ) => { const targetConversationId = chatConversationsRef.current.some((conversation) => conversation.id === request.chatId) ? request.chatId : activeConversationIdRef.current; if (!targetConversationId) { setBannerSeverity('error'); setBannerMessage(t.sections.chat.sendFailed('conversation_not_found')); return; }
...
const handleForgerLogout = async () => { setForgerAccountBusy(true); try { const result = await getDesktopApi().logoutForgerAccount(); setForgerAccount(result); setForumParticipation(initialForumParticipation); setForumPromptOpen(false); setRemoteBackups([]); setRemoteBackupsUsage(initialRemoteBackupsUsage); setForgerAccountMessage(null); await refreshApps(); } finally { setForgerAccountBusy(false); }
};
