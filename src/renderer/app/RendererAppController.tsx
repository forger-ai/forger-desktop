...
const [activeConversationByApp, setActiveConversationByApp] = useState<Record<string, string>>( persistedChatState.activeConversationByApp, ); const [activeConversationId, setActiveConversationId] = useState<string | null>( persistedChatState.lastActiveConversationId, ); const [chatRunActive, setChatRunActive] = useState(false); const [chatProgressLines, setChatProgressLines] = useState<string[]>([]); const activeRunConversationIdRef = useRef<string | null>(null);
const selectedAutomationIdRef = useRef<string | null>(null); const runConversationIdByRunRef = useRef<Map<string, string>>(new Map()); const deliveredRunRepliesRef = useRef<Set<string>>(new Set()); const [bannerMessage, setBannerMessage] = useState<string | null>(null); const [bannerSeverity, setBannerSeverity] = useState<AlertColor>('success'); const [catalogFilter, setCatalogFilter] = useState<'all' | AppCategory>('all');
const [catalogStatusFilter, setCatalogStatusFilter] = useState<'all' | 'installed' | 'not_installed'>('all'); const [selectedToolsTool, setSelectedToolsTool] = useState<SelectedToolsTool>(null); const [earlyAccessEnabled, setEarlyAccessEnabled] = useState(() => readStoredBoolean(EARLY_ACCESS_STORAGE_KEY)); const [advancedMode, setAdvancedMode] = useState(() => readStoredBoolean(ADVANCED_MODE_STORAGE_KEY)); const [themePreference, setThemePreference] = useState<ThemePreference>(getStoredThemePreference);
...
const performInstall = async (appId: string) => { try { const desktopApi = getDesktopApi(); const result = await desktopApi.installApp(appId, activeLocale); await refreshApps(); if (result.success) { setBannerSeverity('success'); setBannerMessage(result.userMessage || t.banners.installed(getAppMeta(appId).name)); const welcome = await desktopApi.installWelcome(appId, activeLocale); if (welcome.success && welcome.message) { createInstallWelcomeConversation(appId, welcome.message); setSelectedAppId(appId);
setCurrentView('chat'); }
} else { setBannerSeverity('error'); setBannerMessage(result.userMessage); requestErrorReportFromResult('app', 'install', result, { appId, appVersion: installedApps.find((appEntry) => appEntry.id === appId)?.version, }); }
...
const refreshCodexAuthStatus = async () => { const desktopApi = getDesktopApi(); const nextStatus = await desktopApi.getCodexAuthStatus(); setCodexAuthStatus(nextStatus); };
const refreshClaudeAuthStatus = async () => { const desktopApi = getDesktopApi(); const nextStatus = await desktopApi.getClaudeAuthStatus(); setClaudeAuthStatus(nextStatus); };
const handleConnectCodexAuth = async () => { setCodexAuthBusy(true); try { const desktopApi = getDesktopApi(); const result = await desktopApi.connectCodexAuth(); setBannerSeverity(result.success ? 'info' : 'error'); setBannerMessage(result.userMessage); requestErrorReportFromResult('agent', 'codex-connect', result); await refreshCodexAuthStatus(); setAgentProviderConfigOpen(false); } catch (error) { setBannerSeverity('error'); setBannerMessage(t.settings.codexConnectError); requestErrorReport({ source: 'agent',
operation: 'codex-connect', message: error instanceof Error ? error.message : t.settings.codexConnectError, technicalCode: 'codex_connect_unhandled_error', sensitiveDetails: { stack: error instanceof Error ? error.stack : undefined }, }); } finally { setCodexAuthBusy(false); }
};
const handleConnectClaudeAuth = async () => { setCodexAuthBusy(true); try { const result = await getDesktopApi().connectClaudeAuth(); setBannerSeverity(result.success ? 'info' : 'error'); setBannerMessage(result.userMessage); if (result.status) { setClaudeAuthStatus(result.status); } else { await refreshClaudeAuthStatus(); }
setAgentProviderConfigOpen(false); requestErrorReportFromResult('agent', 'claude-connect', result); } catch (error) { setBannerSeverity('error'); setBannerMessage('No pudimos iniciar la conexion con Claude Code.'); requestErrorReport({ source: 'agent', operation: 'claude-connect', message: error instanceof Error ? error.message : 'No pudimos iniciar la conexion con Claude Code.', technicalCode: 'claude_connect_unhandled_error', sensitiveDetails: { stack: error instanceof Error ? error.stack : undefined }, });
...
return result; };
const handleSubmitFeedback = async (input: SubmitProductFeedbackInput) => { const result = await getDesktopApi().submitProductFeedback({ ...input, locale: t.locale }); setBannerSeverity(result.success ? 'success' : 'error'); setBannerMessage(result.userMessage ?? t.settings.authErrorFallback); return result; };
