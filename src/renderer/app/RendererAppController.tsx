...
const persistedChatState = useMemo(() => readPersistedChatState(), []); const socialChatWindowRoute = useMemo(() => resolveSocialChatWindowRoute(), []); const [currentView, setCurrentView] = useState<View>('my-apps'); const [installedApps, setInstalledApps] = useState<AppSummary[]>([]); const [memories, setMemories] = useState<MemoryEntry[]>([]); const [catalogApps, setCatalogApps] = useState<CatalogApp[]>([]); const [openingAppIds, setOpeningAppIds] = useState<Set<string>>(new Set());
const openingAppIdsRef = useRef<Set<string>>(new Set()); const installedAppsRef = useRef<AppSummary[]>([]); const [installProgressByApp, setInstallProgressByApp] = useState<Record<string, InstallAppResult>>({}); const [settings, setSettings] = useState<Settings>(initialSettings); const [codexAuthBusy, setCodexAuthBusy] = useState(false); const [codexAuthStatus, setCodexAuthStatus] = useState<CodexAuthStatus>(initialCodexAuthStatus);
...
const [activeConversationByApp, setActiveConversationByApp] = useState<Record<string, string>>( persistedChatState.activeConversationByApp, ); const [activeConversationId, setActiveConversationId] = useState<string | null>( persistedChatState.lastActiveConversationId, ); const [chatRunActive, setChatRunActive] = useState(false); const [activeChatRunId, setActiveChatRunId] = useState<string | null>(null); const [chatProgressLines, setChatProgressLines] = useState<string[]>([]); const activeRunConversationIdRef = useRef<string | null>(null);
const activeChatRunIdRef = useRef<string | null>(null); const selectedAutomationIdRef = useRef<string | null>(null); const runConversationIdByRunRef = useRef<Map<string, string>>(new Map()); const deliveredRunRepliesRef = useRef<Set<string>>(new Set()); const [bannerMessage, setBannerMessage] = useState<string | null>(null); const [bannerSeverity, setBannerSeverity] = useState<AlertColor>('success'); const [catalogFilter, setCatalogFilter] = useState<'all' | AppCategory>('all');
const [catalogStatusFilter, setCatalogStatusFilter] = useState<'all' | 'installed' | 'not_installed'>('all'); const [selectedToolsTool, setSelectedToolsTool] = useState<SelectedToolsTool>(null); const [earlyAccessEnabled, setEarlyAccessEnabled] = useState(() => readStoredBoolean(EARLY_ACCESS_STORAGE_KEY)); const [advancedMode, setAdvancedMode] = useState(() => readStoredBoolean(ADVANCED_MODE_STORAGE_KEY)); const [usageAnalyticsEnabled, setUsageAnalyticsEnabled] = useState(getUsageAnalyticsEnabled); const [themePreference, setThemePreference] = useState<ThemePreference>(getStoredThemePreference);
...
return; }
const isMessageTerminal = run.status === 'preview_ready' || run.status === 'applied' || run.status === 'undone' || run.status === 'failed' || run.status === 'canceled'; if (isTerminal && !isMessageTerminal) { activeRunConversationIdRef.current = null; }
...
}), ); if (isTerminal) { runConversationIdByRunRef.current.delete(run.runId); activeRunConversationIdRef.current = null; }
...
}, []); useEffect(() => { selectedAutomationIdRef.current = selectedAutomationId; }, [selectedAutomationId]); useEffect(() => { installedAppsRef.current = installedApps; }, [installedApps]); useEffect(() => { if (typeof window === 'undefined') { return; }
...
const getCategoryLabel = (category: AppCategory) => t.appCategories[category]; const setChatContext = (appId: string) => { const appEntry = installedApps.find((app) => app.id === appId) ?? catalogApps.find((app) => app.id === appId); if (!appEntry) { return; }
setSelectedAppId(appEntry.id); setCurrentView('chat'); setChatInput(''); setChatProgressLines([]); deliveredRunRepliesRef.current.clear(); };
...
const handleSelectChatApp = (appId: string | null) => { if (!appId) { setSelectedAppId(null); setChatInput(''); setChatProgressLines([]); deliveredRunRepliesRef.current.clear(); return; }
...
const handleDeleteApp = async (appId: string) => { const meta = getAppMeta(appId); const confirmed = window.confirm(t.appView.deleteConfirm(meta.name)); if (!confirmed) { return; }
const desktopApi = getDesktopApi(); const result = await desktopApi.uninstallApp(appId); await refreshApps(); if (selectedAppDetailsId === appId) { setSelectedAppDetails(null); setCurrentView('my-apps'); }
...
: resolveChatRuntimeDraft( resolvedChatProvider, selectedCodexModel, selectedCodexReasoningEffort, selectedClaudeModel, selectedClaudeEffort, ); setActiveConversationByApp((current) => ({ ...current, [chatScopeId]: targetConversationId as string })); setChatRunActive(true); activeChatRunIdRef.current = null; setActiveChatRunId(null); activeRunConversationIdRef.current = targetConversationId; try { const desktopApi = getDesktopApi(); const stagedFilesForCleanup = pendingChatFiles.filter((file) => file.staged); const importedFiles = pendingChatFiles.length > 0
...
setSelectedAppId(target.appId === FREE_CHAT_APP_ID ? null : target.appId); setCurrentView('chat'); setActiveConversationId(target.id); setActiveConversationByApp((current) => ({ ...current, [target.appId]: target.id })); setChatProgressLines([]); deliveredRunRepliesRef.current.clear(); };
...
if (activeRunConversationIdRef.current === conversationId) { activeRunConversationIdRef.current = null; }
};
...
setChatConversations((current) => [nextConversation, ...current]); setActiveConversationId(nextConversation.id); setActiveConversationByApp((current) => ({ ...current, [chatScopeId]: nextConversation.id })); setChatInput(''); discardStagedChatFiles(pendingChatFiles); setPendingChatFiles([]); setMentionedChatFileIds([]); setChatProgressLines([]); deliveredRunRepliesRef.current.clear(); };
...
const pollCodexAuthStatus = async (shouldStop: () => boolean) => { const deadline = Date.now() + AUTH_STATUS_POLL_TIMEOUT_MS; let lastStatus = await refreshCodexAuthStatus(); while (!lastStatus.authenticated && Date.now() < deadline && !shouldStop()) { await wait(AUTH_STATUS_POLL_INTERVAL_MS); lastStatus = await refreshCodexAuthStatus(); }
return lastStatus; };
const pollClaudeAuthStatus = async (shouldStop: () => boolean) => { const deadline = Date.now() + AUTH_STATUS_POLL_TIMEOUT_MS; let lastStatus = await refreshClaudeAuthStatus(); while (!lastStatus.authenticated && Date.now() < deadline && !shouldStop()) { await wait(AUTH_STATUS_POLL_INTERVAL_MS); lastStatus = await refreshClaudeAuthStatus(); }
return lastStatus; };
const handleConnectCodexAuth = async () => { setCodexAuthBusy(true); let connectSettled = false; const polling = pollCodexAuthStatus(() => connectSettled).catch(() => undefined); try { const desktopApi = getDesktopApi(); const result = await desktopApi.connectCodexAuth(); connectSettled = true; await polling; setBannerSeverity(result.success ? 'info' : 'error'); setBannerMessage(result.userMessage); requestErrorReportFromResult('agent', 'codex-connect', result); const nextStatus = result.success ? await pollCodexAuthStatus(() => false) : await refreshCodexAuthStatus(); if (nextStatus.authenticated) { setAgentProviderConfigOpen(false); } } catch (error) { connectSettled = true; await polling; setBannerSeverity('error'); setBannerMessage(t.settings.codexConnectError); requestErrorReport({ source: 'agent',
operation: 'codex-connect', message: error instanceof Error ? error.message : t.settings.codexConnectError, technicalCode: 'codex_connect_unhandled_error', sensitiveDetails: { stack: error instanceof Error ? error.stack : undefined }, }); } finally { connectSettled = true; setCodexAuthBusy(false); }
};
const handleConnectClaudeAuth = async () => { setCodexAuthBusy(true); let connectSettled = false; const polling = pollClaudeAuthStatus(() => connectSettled).catch(() => undefined); try { const result = await getDesktopApi().connectClaudeAuth(); connectSettled = true; await polling; setBannerSeverity(result.success ? 'info' : 'error'); setBannerMessage(result.userMessage); if (result.status) { setClaudeAuthStatus(result.status); }
const nextStatus = result.success ? await pollClaudeAuthStatus(() => false) : result.status ?? await refreshClaudeAuthStatus(); if (nextStatus.authenticated) { setAgentProviderConfigOpen(false); }
requestErrorReportFromResult('agent', 'claude-connect', result); } catch (error) { connectSettled = true; await polling; setBannerSeverity('error'); setBannerMessage('No pudimos iniciar la conexion con Claude Code.'); requestErrorReport({ source: 'agent', operation: 'claude-connect', message: error instanceof Error ? error.message : 'No pudimos iniciar la conexion con Claude Code.', technicalCode: 'claude_connect_unhandled_error', sensitiveDetails: { stack: error instanceof Error ? error.stack : undefined }, });
} finally { connectSettled = true; setCodexAuthBusy(false); }
};
const handleReinstallClaude = async () => { setCodexAuthBusy(true); try { const result = await getDesktopApi().reinstallClaude(); setBannerSeverity(result.success ? 'success' : 'error'); setBannerMessage(result.userMessage); if (result.status) { setClaudeAuthStatus(result.status); } else { await refreshClaudeAuthStatus(); }
requestErrorReportFromResult('agent', 'claude-reinstall', result); } catch (error) { setBannerSeverity('error'); setBannerMessage('No pudimos instalar Claude Code.'); requestErrorReport({ source: 'agent', operation: 'claude-reinstall', message: error instanceof Error ? error.message : 'No pudimos instalar Claude Code.', technicalCode: 'claude_reinstall_unhandled_error', sensitiveDetails: { stack: error instanceof Error ? error.stack : undefined }, }); } finally { setCodexAuthBusy(false); }
};
const handleReinstallCodex = async () => { setCodexAuthBusy(true); try { const desktopApi = getDesktopApi(); const result = await desktopApi.reinstallCodex(); setBannerSeverity(result.success ? 'success' : 'error'); setBannerMessage(result.userMessage); if (result.status) { setCodexAuthStatus(result.status); } else { await refreshCodexAuthStatus(); }
...
handleDeleteRemoteBackup, handleRestoreBackup, handleRestoreRemoteBackup, handleSetAutoSync, openCloudUpsell, userSecrets, handleCreateSecret, handleUpdateSecret, handleDeleteSecret, agentToolPackages, agentToolSettings, officialTools, selectedToolsTool, agentToolBusyId, officialToolBusyId, agentToolError, agentToolErrorCode, setSelectedToolsTool, handleAgentToolApprovalChange, runOfficialToolAction, activeLocale, codexAuthBusy, themePreference, setThemePreference, languagePreference, systemLocale,
setLanguagePreference, chatBotPicture, setChatBotPicture, handleAgentDefaultsChange, setCodexConfigOpen, handleReinstallCodex, setClaudeConfigOpen, handleReinstallClaude, desktopUpdateBusy, runDesktopUpdateAction, memories, handleCreateMemory, handleUpdateMemory, handleDeleteMemory, cloudIdentity, setCloudIdentity, setEarlyAccessEnabled, setAdvancedMode, pendingInstallGate, pendingInstallBusy, setPendingInstallGate, renderInstallTool, renderInstallItem, handleConfirmInstallWithTools, cloudModalOpen,
...
handleMoveFileSubmit, errorReportDialog, closeErrorReportDialog, copyErrorReportDetails, submitErrorReport, bannerMessage, bannerSeverity };
}
