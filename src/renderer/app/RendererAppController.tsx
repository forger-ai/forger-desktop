...
runConversationIdByRunRef.current.set(startResult.runId, targetConversationId); submitUsageEvent({ eventName: 'chat_started', surface: 'chat', locale: t.locale, stringParameters: { app_id: selectedAppId ?? FREE_CHAT_APP_ID, provider: runtimeDraft.provider } }); } catch (error) { setChatRunActive(false); activeRunConversationIdRef.current = null; const detail = error instanceof Error ? error.message : t.settings.authErrorFallback; const friendly = /another_run_in_progress/i.test(detail) ? t.sections.chat.sendInProgress : t.sections.chat.sendFailed(detail); setChatConversations((currentConversations) => currentConversations.map((conversation) => { if (conversation.id !== targetConversationId) {
return conversation; }
