...
settle(desktopApi.getSettings(), (value) => { setSettings(value); setSelectedChatPermissionMode(value.defaultChatPermissionMode ?? 'safe'); setSelectedChatNetworkAccess(value.defaultChatNetworkAccess !== false); setSelectedAntigravityModel(value.agentDefaults.antigravity.model); setSelectedAntigravityEffort(value.agentDefaults.antigravity.effort); });
...
const handleDisconnectCodexAuth = async () => { setAuthBusyProvider('codex'); try { const result = await getDesktopApi().disconnectCodexAuth(); setBannerSeverity(result.success ? 'success' : 'error'); setBannerMessage(result.userMessage); requestErrorReportFromResult('agent', 'codex-disconnect', result); await refreshCodexAuthStatus(); const updatedSettings = await getDesktopApi().getSettings(); setSettings(updatedSettings); } catch (error) { setBannerSeverity('error'); setBannerMessage(t.settings.codexDisconnectError); requestErrorReport({ source: 'agent', operation: 'codex-disconnect', message: error instanceof Error ? error.message : t.settings.codexDisconnectError, technicalCode: 'codex_disconnect_unhandled_error', sensitiveDetails: { stack: error instanceof Error ? error.stack : undefined }, }); } finally { setAuthBusyProvider((current) => current === 'codex' ? null : current); }
};
const handleDisconnectClaudeAuth = async () => { setAuthBusyProvider('claude'); try { const result = await getDesktopApi().disconnectClaudeAuth(); setBannerSeverity(result.success && !result.status?.authenticated ? 'success' : 'warning'); setBannerMessage(result.userMessage); if (result.status) { setClaudeAuthStatus(result.status); } else { await refreshClaudeAuthStatus(); }
requestErrorReportFromResult('agent', 'claude-disconnect', result); const updatedSettings = await getDesktopApi().getSettings(); setSettings(updatedSettings); } catch (error) { setBannerSeverity('error'); setBannerMessage('No pudimos desconectar Claude Code.'); requestErrorReport({ source: 'agent', operation: 'claude-disconnect', message: error instanceof Error ? error.message : 'No pudimos desconectar Claude Code.', technicalCode: 'claude_disconnect_unhandled_error', sensitiveDetails: { stack: error instanceof Error ? error.stack : undefined }, }); } finally { setAuthBusyProvider((current) => current === 'claude' ? null : current); }
};
const handleConnectAntigravityAuth = async () => { setAuthBusyProvider('antigravity'); setAntigravityConfigOpen(false); setAntigravityAuthConsoleOpen(true); setAntigravityAuthSessionId(null); setAntigravityAuthLines([]); antigravityAuthPollingCanceledRef.current = false; try { const result = await getDesktopApi().startAntigravityAuthSession(); setBannerSeverity(result.success ? 'info' : 'error'); setBannerMessage(result.userMessage); if (result.status) { setAntigravityAuthStatus(result.status); }
if (result.sessionId) { setAntigravityAuthSessionId(result.sessionId); appendAntigravityAuthLine('system', result.userMessage); } else { setAuthBusyProvider((current) => current === 'antigravity' ? null : current); }
requestErrorReportFromResult('agent', 'antigravity-connect', result); if (!result.success) { return; }
const nextStatus = result.status?.authenticated ? result.status : await pollAntigravityAuthStatus(() => antigravityAuthPollingCanceledRef.current); if (antigravityAuthPollingCanceledRef.current) { return; }
setAntigravityAuthStatus(nextStatus); if (nextStatus.authenticated) { setBannerSeverity('success'); setBannerMessage(nextStatus.userMessage ?? 'Google Antigravity está conectado.'); setAntigravityConfigOpen(false); setAgentProviderConfigOpen(false); setAntigravityAuthConsoleOpen(false); setAntigravityAuthSessionId(null); } else { setBannerSeverity('info'); setBannerMessage('Antigravity sigue esperando el código en Terminal. Puedes cerrar esta ventana y actualizar el estado después.'); }
} catch (error) { setBannerSeverity('error'); setBannerMessage('No pudimos iniciar la conexión con Google Antigravity.'); requestErrorReport({ source: 'agent', operation: 'antigravity-connect', message: error instanceof Error ? error.message : 'No pudimos iniciar la conexión con Google Antigravity.', technicalCode: 'antigravity_connect_unhandled_error', sensitiveDetails: { stack: error instanceof Error ? error.stack : undefined }, }); } finally { setAuthBusyProvider((current) => current === 'antigravity' ? null : current); }
};
...
setLanguagePreference, chatBotPicture, setChatBotPicture, handleAgentDefaultsChange, handleDeveloperModeChange, setCodexConfigOpen, closeCodexConfig, handleDisconnectCodexAuth, handleReinstallCodex, setClaudeConfigOpen, closeClaudeConfig, handleDisconnectClaudeAuth, handleReinstallClaude, antigravityConfigOpen, setAntigravityConfigOpen, closeAntigravityConfig, handleDisconnectAntigravityAuth, handleReinstallAntigravity, desktopUpdateBusy, runDesktopUpdateAction, memories, handleCreateMemory, handleUpdateMemory, handleDeleteMemory, cloudIdentity, setCloudIdentity, setEarlyAccessEnabled, setAdvancedMode, pendingInstallGate, pendingInstallBusy, setPendingInstallGate, renderInstallTool, renderInstallItem, handleConfirmInstallWithTools, cloudModalOpen,
