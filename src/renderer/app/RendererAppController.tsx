...
const chatBotPictureSrc = CHAT_BOT_PICTURE_OPTIONS.find((option) => option.value === chatBotPicture)?.src ?? CHAT_BOT_PICTURE_OPTIONS[0].src; const activeConversation = useMemo( () => chatConversations.find((conversation) => conversation.id === activeConversationId) ?? null, [chatConversations, activeConversationId], ); const chatMessages = activeConversation?.messages ?? []; const activeConversationRun = activeConversationId ? activeChatRunsByConversation[activeConversationId] ?? null : null; const chatRunActive = Object.keys(activeChatRunsByConversation).length > 0 || startingConversationIds.size > 0; const activeConversationRunActive = Boolean(activeConversationRun || (activeConversationId && startingConversationIds.has(activeConversationId))); const activeConversationRunId = activeConversationRun?.runId ?? null; const activeConversationProgressLines = activeConversationRun?.progressLog ?? []; const mentionedChatFiles = useMemo( () => forgerFiles.filter((file) => mentionedChatFileIds.includes(file.id)),
...
if (!deliveredRunRepliesRef.current.has(dedupePermissionKey)) { deliveredRunRepliesRef.current.add(dedupePermissionKey); const targetConversationId = runConversationId ?? runConversationIdByRunRef.current.get(run.runId) ?? activeConversationIdRef.current; if (targetConversationId) { const permissionRequest = run.permissionRequest; const messageId = `assistant-permission-${run.runId}-${permissionRequest.requestId}`; console.info('[Forger permission] rendering request', { runId: run.runId, requestId: permissionRequest.requestId, permission: permissionRequest.permission,
...
return; }
deliveredRunRepliesRef.current.add(dedupeKey); const targetConversationId = runConversationId ?? runConversationIdByRunRef.current.get(run.runId) ?? activeConversationIdRef.current; if (!targetConversationId) { traceChatEvent({ event: 'chat_run_message_append_attempt', runId: run.runId, appId: run.appId, conversationId: null, activeConversationId: activeConversationIdRef.current, status: run.status, foundConversation: false }); return; }
...
}), ); traceChatEvent({ event: 'chat_run_message_appended', runId: run.runId, appId: run.appId, conversationId: targetConversationId, activeConversationId: activeConversationIdRef.current, status: run.status, foundConversation }); if (isTerminal) { runConversationIdByRunRef.current.delete(run.runId); }
};
...
if ((!trimmed && pendingChatFiles.length === 0 && mentionedChatFileIds.length === 0) || activeConversationRunActive || !hasAgentProvider) { if (!hasAgentProvider) { setAgentProviderConfigOpen(true); }
return; }
...
}), ); requestErrorReport({ source: 'agent', operation: 'chat.start-run', message: detail, technicalCode: 'chat_start_run_failed', appId: selectedAppId ?? undefined, appVersion: installedApps.find((appEntry) => appEntry.id === selectedAppId)?.version, sensitiveDetails: { stack: error instanceof Error ? error.stack : undefined }, }); } finally { setStartingConversationIds((current) => { const next = new Set(current); next.delete(targetConversationId as string); return next; }); }
};
...
handleRemovePendingChatFile, handleMentionFile, setMentionedChatFileIds, activeConversation, selectedAgentProvider, resolvedChatProvider, setSelectedAgentProvider, selectedCodexModel, setSelectedCodexModel, selectedCodexReasoningEffort, setSelectedCodexReasoningEffort, selectedClaudeModel, setSelectedClaudeModel, selectedClaudeEffort, setSelectedClaudeEffort, chatBotPictureSrc, activeConversationRunActive, activeConversationRunId, activeConversationProgressLines, codexAuthStatus, claudeAuthStatus, setAgentProviderConfigOpen, handleStopChatRun, handleRespondPermission, prepareConversationDiagnosticReport, conversationDiagnosticDialog, closeConversationDiagnosticDialog, copyConversationDiagnosticReport, submitConversationDiagnosticReport, automations,
