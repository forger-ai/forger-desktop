...
const [catalogStatusFilter, setCatalogStatusFilter] = useState<'all' | 'installed' | 'not_installed'>('all'); const [selectedToolsTool, setSelectedToolsTool] = useState<SelectedToolsTool>(null); const [localNetworkShareDialogOpen, setLocalNetworkShareDialogOpen] = useState(false); const [localNetworkShareStatus, setLocalNetworkShareStatus] = useState<LocalNetworkShareStatus | null>(null); const [earlyAccessEnabled, setEarlyAccessEnabled] = useState(() => readStoredBoolean(EARLY_ACCESS_STORAGE_KEY)); const [advancedMode, setAdvancedMode] = useState(() => readStoredBoolean(ADVANCED_MODE_STORAGE_KEY)); const [usageAnalyticsEnabled, setUsageAnalyticsEnabled] = useState(getUsageAnalyticsEnabled); const [themePreference, setThemePreference] = useState<ThemePreference>(getStoredThemePreference);
...
setBannerSeverity, setBannerMessage, handleForgerLogout, desktopUpdateState, advancedMode, openingAppIds, getCategoryLabel, handleOpen, handleStartLocalNetworkShare, handleStopLocalNetworkShare, localNetworkShareDialogOpen, setLocalNetworkShareDialogOpen, localNetworkShareStatus, handleStop, handleRetry, handleUpdate, handleRestoreUserVersion, handleResolveConflict, openAppDetails, handleDeleteApp, installProgressByApp, catalogApps, refreshApps, catalogFilter, setCatalogFilter, catalogStatusFilter, setCatalogStatusFilter, handleInstall, earlyAccessEnabled, selectedAppDetails, selectedAppDetailsId, appSecretsState, secretsBusy, settings, appDetailsBackView,
