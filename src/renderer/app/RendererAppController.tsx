...
const buildOptionalGrantDrafts = (gate: AppToolsInstallGate | null): Record<string, boolean> => Object.fromEntries((gate?.optional ?? []).map((item) => [item.declaration.toolId, item.hasStoredGrant ? item.granted : true]));
const gateHasReviewableAccess = (gate: AppToolsInstallGate | null): boolean => Boolean(gate && (Object.keys(gate.platformCapabilities ?? {}).length > 0 || gate.required.length > 0 || gate.optional.length > 0 || gate.agents.length > 0 || gate.promptTemplates.length > 0));
const persistOptionalGrantDrafts = async (appId: string, gate: AppToolsInstallGate | null, drafts: Record<string, boolean>) => { if (!gate) { return; } await Promise.all(gate.optional.map((item) => getDesktopApi().setAppToolGrant({ appId, toolId: item.declaration.toolId, granted: drafts[item.declaration.toolId] ?? false }, activeLocale))); };
...
setPendingInstallGrantDrafts((current) => ({ ...current, [toolId]: granted }));
};
const handleSocialOptionalToolGrant = (toolId: string, granted: boolean) => setSocialInstallReviewDialog((current) => ({ ...current, grantDrafts: { ...(current.grantDrafts ?? {}), [toolId]: granted } }));
...
const renderInstallTool = (item: AppToolsInstallGate['required'][number] | AppToolsInstallGate['optional'][number], required: boolean, grantDrafts: Record<string, boolean> = pendingInstallGrantDrafts, onDraftChange: (toolId: string, granted: boolean) => void = handleOptionalToolGrant) => { const configured = item.available && item.configured; const statusLabel = configured ? t.installGate.toolActive : item.available ? t.installGate.toolNeedsConfiguration : t.installGate.toolInactive; const actionLabels = item.allActions && item.resolvedActions.length === 0 ? [t.installGate.allActions] : item.resolvedActions.map((action) => action.name); const checked = grantDrafts[item.declaration.toolId] ?? item.granted; return ( <Paper key={`${required ? 'required' : 'optional'}-${item.declaration.toolId}`} variant="outlined"
