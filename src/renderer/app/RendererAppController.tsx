...
const initialSettings: Settings = { userEmail: '', plan: 'Free', safeMode: false, developerMode: { enabled: false, pathEntries: [] }, codexDefaults: { ...DEFAULT_AGENT_DEFAULTS.codex }, defaultAgentProvider: 'auto', defaultChatPermissionMode: 'safe', defaultChatNetworkAccess: true, agentDefaults: { codex: { ...DEFAULT_AGENT_DEFAULTS.codex }, claude: { ...DEFAULT_AGENT_DEFAULTS.claude }, antigravity: { ...DEFAULT_AGENT_DEFAULTS.antigravity }, }, llmProviderDefaults: { codex: { ...DEFAULT_AGENT_DEFAULTS.codex }, claude: { ...DEFAULT_AGENT_DEFAULTS.claude }, antigravity: { ...DEFAULT_AGENT_DEFAULTS.antigravity }, }, providerConnections: {}, };
...
const resolveChatRuntimeDraft = ( provider: AgentProvider, codexModel: string, codexReasoningEffort: CodexReasoningEffort, claudeModel: string, claudeEffort: ClaudeEffort, antigravityModel: string, antigravityEffort: AgentEffort, ): { provider: AgentProvider; model: string; effort: AgentEffort; reasoningEffort?: CodexReasoningEffort } => { if (provider === 'codex') { return { provider: 'codex', model: codexModel, reasoningEffort: codexReasoningEffort, effort: codexReasoningEffort }; }
...
const socialAppDetails = (app: SocialUserApp, shareCode?: string, installedApp?: AppSummary): AppDetails => {
  ...
  return {
    app: {
      ...
      category: installedApp?.category ?? 'productividad',
      status,
      latestVersion,
...
open: boolean; appId: string | null; visibility: SocialUploadVisibility; name: string; slug: string; isRemix: boolean; }
...
const [socialUploadDialog, setSocialUploadDialog] = useState<SocialUploadDialogState>({ open: false, appId: null, visibility: 'private', name: '', slug: '', isRemix: false });
...
if (claudeAuthStatus.authenticated && !codexAuthStatus.authenticated && !antigravityAuthStatus.authenticated) { return 'claude'; }
if (antigravityAuthStatus.authenticated && !codexAuthStatus.authenticated && !claudeAuthStatus.authenticated) { return 'antigravity'; }
if (codexAuthStatus.authenticated && !claudeAuthStatus.authenticated && !antigravityAuthStatus.authenticated) { return 'codex'; }
const connectedProviders = (Object.entries(settings.providerConnections) as Array<[AgentProvider, string | undefined]>) .filter(([, connectedAt]) => Boolean(connectedAt)) .sort(([, left], [, right]) => Date.parse(left ?? '') - Date.parse(right ?? '')); return connectedProviders[0]?.[0] ?? 'codex'; }, [ activeConversation?.runtime, antigravityAuthStatus.authenticated, claudeAuthStatus.authenticated, codexAuthStatus.authenticated, selectedAgentProvider, settings.defaultAgentProvider, settings.providerConnections, ]);
...
const handleAntigravityAuthSessionEvent = (event: AntigravityAuthSessionEvent) => { if (event.sessionId !== antigravityAuthSessionId) { return; }
...
if (event.type === 'url' && event.url) { setAntigravityAuthUrl(event.url); }
if (event.type === 'completed') { if (event.status) { setAntigravityAuthStatus(event.status); }
setAuthBusyProvider((current) => current === 'antigravity' ? null : current); setBannerSeverity('success'); setBannerMessage(event.userMessage ?? 'Google Antigravity está conectado.'); void refreshAntigravityAuthStatus(); }
if (event.type === 'failed' || event.type === 'canceled') { setAuthBusyProvider((current) => current === 'antigravity' ? null : current); setBannerSeverity(event.type === 'failed' ? 'error' : 'info'); setBannerMessage(event.userMessage ?? (event.type === 'failed' ? 'No pudimos completar la conexión con Google Antigravity.' : 'Conexión de Google Antigravity cancelada.')); if (event.text) { appendAntigravityAuthLine(event.stream ?? 'system', event.text); } }
};
...
settle(refreshApps(), () => undefined, () => { setBannerSeverity('error'); setBannerMessage(t.settings.authErrorFallback); });
settle(desktopApi.getSettings(), (value) => { setSettings(value); setSelectedChatPermissionMode(value.defaultChatPermissionMode ?? 'safe'); setSelectedChatNetworkAccess(value.defaultChatNetworkAccess !== false); });
...
settle(desktopApi.getCodexAuthStatus(), setCodexAuthStatus);
settle(desktopApi.getClaudeAuthStatus(), setClaudeAuthStatus);
settle(desktopApi.getAntigravityAuthStatus(), setAntigravityAuthStatus);
...
settle(refreshOfficialTools(), () => undefined, () => { setAgentToolError(t.sections.tools.saveError); });
...
const submitSocialUploadDialog = async () => { const { appId, visibility, name, slug, isRemix } = socialUploadDialog; if (!appId) { return; }
setSocialUploadDialog((current) => ({ ...current, open: false })); void uploadSocialApp(appId, visibility, isRemix ? { name: name.trim(), slug } : {});
};
...
const handleAgentDefaultsChange = async (input: UpdateAgentDefaultsInput) => { setSettings((current) => { const currentAgentDefaults = { codex: current.agentDefaults.codex ?? DEFAULT_AGENT_DEFAULTS.codex, claude: current.agentDefaults.claude ?? DEFAULT_AGENT_DEFAULTS.claude, antigravity: current.agentDefaults.antigravity ?? DEFAULT_AGENT_DEFAULTS.antigravity, }; const nextDefaultProvider = input.defaultProvider ?? current.defaultAgentProvider; const nextDefaultChatPermissionMode = input.defaultChatPermissionMode ?? current.defaultChatPermissionMode; const nextDefaultChatNetworkAccess = input.defaultChatNetworkAccess ?? current.defaultChatNetworkAccess; if (!input.provider) { return { ...current, agentDefaults: currentAgentDefaults, defaultAgentProvider: nextDefaultProvider, defaultChatPermissionMode: nextDefaultChatPermissionMode, defaultChatNetworkAccess: nextDefaultChatNetworkAccess }; }
...
if (input.provider === 'claude') { return { ...current, defaultAgentProvider: nextDefaultProvider, defaultChatPermissionMode: nextDefaultChatPermissionMode, defaultChatNetworkAccess: nextDefaultChatNetworkAccess, agentDefaults: { ...currentAgentDefaults, claude: { model: input.model ?? currentAgentDefaults.claude.model, effort: (input.effort as ClaudeEffort | undefined) ?? currentAgentDefaults.claude.effort, }, }, };
}
return { ...current, defaultAgentProvider: nextDefaultProvider, defaultChatPermissionMode: nextDefaultChatPermissionMode, defaultChatNetworkAccess: nextDefaultChatNetworkAccess, agentDefaults: { ...currentAgentDefaults, antigravity: { model: input.model ?? currentAgentDefaults.antigravity.model, effort: (input.effort as typeof currentAgentDefaults.antigravity.effort | undefined) ?? currentAgentDefaults.antigravity.effort, }, }, };
...
const handleSendMessage = async (nextMessage?: string, modeOverride?: { mode: ChatMode; targetAppId?: string | null }) => { const trimmed = (nextMessage ?? chatInput).trim(); const sharedFileNames = [ ...pendingChatFiles.map((file) => file.name), ...mentionedChatFiles.map((file) => file.name), ]; const userVisibleContent = trimmed || `Archivos compartidos: ${sharedFileNames.join(', ')}`; const hasAgentProvider = codexAuthStatus.authenticated || claudeAuthStatus.authenticated || antigravityAuthStatus.authenticated; const activeChatMode = modeOverride?.mode ?? activeConversation?.mode; const activeTargetAppId = activeChatMode === 'edit_app' ? modeOverride?.targetAppId ?? activeConversation?.targetAppId ?? activeConversation?.appId ?? null : null;
...
: resolveChatRuntimeDraft( resolvedChatProvider, selectedCodexModel, selectedCodexReasoningEffort, selectedClaudeModel, selectedClaudeEffort, settings.agentDefaults.antigravity.model, settings.agentDefaults.antigravity.effort, ); const userVisibleContent = buildQuestionResponseMessage(request, response); const internalPrompt = buildQuestionResponsePrompt(request, response); const now = new Date().toISOString(); setChatConversations((currentConversations) => currentConversations.map((conversation) => { if (conversation.id !== targetConversationId) { return conversation; }
...
const closeAntigravityConfig = () => { cancelAuthConnectAttempt('antigravity'); setAgentProviderConfigOpen(false); };
...
const handleConnectAntigravityAuth = async () => { setAuthBusyProvider('antigravity'); setAntigravityAuthConsoleOpen(true); setAntigravityAuthSessionId(null); setAntigravityAuthUrl(null); setAntigravityAuthCode(''); setAntigravityAuthLines([]); try { const result = await getDesktopApi().startAntigravityAuthSession(); setBannerSeverity(result.success ? 'info' : 'error'); setBannerMessage(result.userMessage); if (result.status) { setAntigravityAuthStatus(result.status); }
if (result.sessionId) { setAntigravityAuthSessionId(result.sessionId); } else { setAuthBusyProvider((current) => current === 'antigravity' ? null : current); }
if (result.status?.authenticated) { setAgentProviderConfigOpen(false); setAntigravityAuthConsoleOpen(false); }
requestErrorReportFromResult('agent', 'antigravity-connect', result); } catch (error) { setAuthBusyProvider((current) => current === 'antigravity' ? null : current); setBannerSeverity('error'); setBannerMessage('No pudimos iniciar la conexión con Google Antigravity.'); requestErrorReport({ source: 'agent', operation: 'antigravity-connect', message: error instanceof Error ? error.message : 'No pudimos iniciar la conexión con Google Antigravity.', technicalCode: 'antigravity_connect_unhandled_error', sensitiveDetails: { stack: error instanceof Error ? error.stack : undefined }, }); }
};
const handleSubmitAntigravityAuthCode = async () => { const code = antigravityAuthCode.trim(); if (!antigravityAuthSessionId || !code) { return; }
const result = await getDesktopApi().writeAntigravityAuthSession({ sessionId: antigravityAuthSessionId, input: code }); if (!result.success) { setBannerSeverity('error'); setBannerMessage(result.userMessage ?? 'No pudimos enviar el código a Google Antigravity.'); requestErrorReportFromResult('agent', 'antigravity-auth-code', result); return; }
setAntigravityAuthCode(''); appendAntigravityAuthLine('system', 'Authorization code sent.'); };
const handleCancelAntigravityAuthSession = async () => { if (antigravityAuthSessionId) { await getDesktopApi().cancelAntigravityAuthSession(antigravityAuthSessionId); }
...
const handleOpenAntigravityAuthUrl = () => { if (antigravityAuthUrl) { void getDesktopApi().openExternalUrl(antigravityAuthUrl); } };
...
const handleReinstallAntigravity = async () => { setAuthBusyProvider('antigravity'); try { const result = await getDesktopApi().reinstallAntigravity(); setBannerSeverity(result.success ? 'success' : 'error'); setBannerMessage(result.userMessage); if (result.status) { setAntigravityAuthStatus(result.status); } else { await refreshAntigravityAuthStatus(); }
requestErrorReportFromResult('agent', 'antigravity-reinstall', result); } catch (error) { setBannerSeverity('error'); setBannerMessage('No pudimos instalar Google Antigravity.'); requestErrorReport({ source: 'agent', operation: 'antigravity-reinstall', message: error instanceof Error ? error.message : 'No pudimos instalar Google Antigravity.', technicalCode: 'antigravity_reinstall_unhandled_error', sensitiveDetails: { stack: error instanceof Error ? error.stack : undefined }, }); } finally { setAuthBusyProvider((current) => current === 'antigravity' ? null : current); }
};
...
handleMoveFileSubmit, remoteTunnelReadyDialog, closeRemoteTunnelReadyDialog, stopReadyRemoteTunnel, openRemoteTunnelPortal, socialUploadDialog, closeSocialUploadDialog, setSocialUploadVisibility, setSocialUploadName, submitSocialUploadDialog, errorReportDialog, closeErrorReportDialog, copyErrorReportDetails, submitErrorReport, bannerMessage, bannerSeverity };
}
