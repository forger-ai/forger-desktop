...
const readStoredBoolean = (key: string, fallback = false) => { const value = window.localStorage.getItem(key); return value === null ? fallback : value === 'true'; };
const readStoredPinnedViews = (): PinnableView[] => { const raw = window.localStorage.getItem(PINNED_VIEWS_STORAGE_KEY); if (raw !== null) { try { const parsed: unknown = JSON.parse(raw); if (Array.isArray(parsed)) { return PINNABLE_VIEWS.filter((view) => parsed.includes(view)); } } catch { /* corrupt value: fall through to legacy migration */ } }
...
const resolveSocialChatWindowRoute = (): SocialChatWindowRoute | null => { const params = new URLSearchParams(window.location.search); if (params.get('socialChat') !== '1') { return null; }
...
const [selectedChatPermissionMode, setSelectedChatPermissionMode] = useState<AgentPermissionMode>(settings.defaultChatPermissionMode);
...
const activeConversationIdRef = useRef<string | null>(activeConversationId); const selectedAppIdRef = useRef<string | null>(selectedAppId); const chatConversationsRef = useRef<ChatConversation[]>(chatConversations); const selectedAutomationIdRef = useRef<string | null>(null); const selectedAutomationRunRef = useRef<AutomationRun | null>(null); const runConversationIdByRunRef = useRef<Map<string, string>>(new Map()); const deliveredRunRepliesRef = useRef<Set<string>>(new Set()); const createdAppUsageEventsRef = useRef<Set<string>>(new Set()); const modifiedAppUsageEventsRef = useRef<Set<string>>(new Set()); const [bannerMessage, setBannerMessage] = useState<string | null>(null); const [bannerSeverity, setBannerSeverity] = useState<AlertColor>('success'); const [catalogFilter, setCatalogFilter] = useState<'all' | AppCategory>('all');
...
const [socialInstallReviewDialog, setSocialInstallReviewDialog] = useState<SocialInstallReviewDialogState>({ open: false, appId: '', appName: '', input: {}, busy: false, grantDrafts: {} });
...
const appendAntigravityAuthLine = (stream: AntigravityAuthConsoleLine['stream'], text: string) => {
...
const refreshForumParticipation = async () => { const state = await getDesktopApi().getForumParticipation(); setForumParticipation(state); setForumPromptOpen(state.status === 'opted_out' && !state.firstPromptShownAt); return state; };
...
loadData(); }, []); useEffect(() => { const desktopApi = getDesktopApi();
...
clearActiveRunState(activeRunToHydrate.runId); }).catch(() => clearActiveRunState(activeRunToHydrate.runId)); } const unsubscribeAutomation = desktopApi.onAutomationUpdated(({ automation, run }) => { setAutomations((current) => { const withoutCurrent = current.filter((item) => item.id !== automation.id); return [automation, ...withoutCurrent].sort((a, b) => Date.parse(b.updatedAt) - Date.parse(a.updatedAt)); }); upsertAutomationBackgroundTask(automation, run); if (selectedAutomationIdRef.current === automation.id) { void desktopApi.automationsListRuns(automation.id).then((runs) => { setAutomationRuns(runs);
	const selectedRunId = selectedAutomationRunRef.current?.id; const targetRunId = run?.id ?? (selectedRunId && runs.some((candidate) => candidate.id === selectedRunId) ? selectedRunId : runs[0]?.id); if (!targetRunId) { setSelectedAutomationRun(null); return; }
...
}, []); useEffect(() => { selectedAutomationIdRef.current = selectedAutomationId; }, [selectedAutomationId]); useEffect(() => { selectedAutomationRunRef.current = selectedAutomationRun; }, [selectedAutomationRun]); useEffect(() => { backgroundTasksRef.current = backgroundTasks; }, [backgroundTasks]); useEffect(() => { activeConversationIdRef.current = activeConversationId; }, [activeConversationId]); useEffect(() => { selectedAppIdRef.current = selectedAppId; }, [selectedAppId]); useEffect(() => { chatConversationsRef.current = chatConversations; }, [chatConversations]); useEffect(() => { installedAppsRef.current = installedApps; }, [installedApps]); useEffect(() => {
...
}); }, [currentView, selectedAppDetailsId, selectedAppDetails?.social?.localAppId, installedApps, catalogApps]); useEffect(() => { window.localStorage.setItem(THEME_STORAGE_KEY, themePreference);
		}, [themePreference]); useEffect(() => { window.localStorage.setItem(LANGUAGE_STORAGE_KEY, languagePreference);
		}, [languagePreference]); useEffect(() => { window.localStorage.setItem(PINNED_VIEWS_STORAGE_KEY, JSON.stringify(pinnedViews)); window.localStorage.setItem(ADVANCED_MODE_STORAGE_KEY, String(pinnedViews.length > 0));
}, [pinnedViews]); useEffect(() => { if (currentView === 'catalog') { submitUsageEvent({ eventName: 'catalog_viewed', surface: 'catalog', locale: t.locale, stringParameters: { filter: catalogFilter, status_filter: catalogStatusFilter } }); } else if (currentView === 'feedback') { submitUsageEvent({ eventName: 'feedback_opened', surface: 'feedback', locale: t.locale, stringParameters: { target: 'product' } }); } }, [catalogFilter, catalogStatusFilter, currentView, t.locale]); useEffect(() => {
const handleLanguageChange = () => setSystemLocale(resolveSystemLocale()); window.addEventListener('languagechange', handleLanguageChange); return () => window.removeEventListener('languagechange', handleLanguageChange); }, []); useEffect(() => { window.localStorage.setItem(CODEX_MODEL_STORAGE_KEY, selectedCodexModel);
}, [selectedCodexModel]); useEffect(() => { window.localStorage.setItem(CODEX_REASONING_STORAGE_KEY, selectedCodexReasoningEffort);
}, [selectedCodexReasoningEffort]); useEffect(() => { window.localStorage.setItem(CHAT_AGENT_PROVIDER_STORAGE_KEY, selectedAgentProvider);
}, [selectedAgentProvider]); useEffect(() => { window.localStorage.setItem(CLAUDE_MODEL_STORAGE_KEY, selectedClaudeModel);
}, [selectedClaudeModel]); useEffect(() => { window.localStorage.setItem(CLAUDE_EFFORT_STORAGE_KEY, selectedClaudeEffort);
}, [selectedClaudeEffort]); useEffect(() => { window.localStorage.setItem(ANTIGRAVITY_MODEL_STORAGE_KEY, selectedAntigravityModel);
}, [selectedAntigravityModel]); useEffect(() => { window.localStorage.setItem(ANTIGRAVITY_EFFORT_STORAGE_KEY, selectedAntigravityEffort);
}, [selectedAntigravityEffort]); useEffect(() => { window.localStorage.setItem(CHAT_BOT_PICTURE_STORAGE_KEY, chatBotPicture);
...
const sessionKey = status.sessionId || `${status.appId}:${status.frontendUrl || status.portalUrl || status.tunnelUrl || ''}`; if (remoteTunnelReadySessionsRef.current.has(sessionKey)) { return; }
...
const handleOptionalGrantDraftChange = async (grantKey: string, granted: boolean) => { setPendingInstallGrantDrafts((current) => ({ ...current, [grantKey]: granted })); };
...
const handleAgentDefaultsChange = async (input: UpdateAgentDefaultsInput) => { setSettings((current) => { const currentAgentDefaults = current.agentDefaults; const nextDefaultProvider = input.defaultProvider ?? current.defaultAgentProvider; const nextDefaultChatPermissionMode = input.defaultChatPermissionMode ?? current.defaultChatPermissionMode; const nextDefaultChatNetworkAccess = input.defaultChatNetworkAccess ?? current.defaultChatNetworkAccess; const nextProviderInactivityTimeoutMinutes = input.provider && input.inactivityTimeoutMinutes !== undefined ? { ...current.providerInactivityTimeoutMinutes, [input.provider]: input.inactivityTimeoutMinutes } : current.providerInactivityTimeoutMinutes; if (!input.provider) { return { ...current, agentDefaults: currentAgentDefaults, defaultAgentProvider: nextDefaultProvider, defaultChatPermissionMode: nextDefaultChatPermissionMode, defaultChatNetworkAccess: nextDefaultChatNetworkAccess, providerInactivityTimeoutMinutes: nextProviderInactivityTimeoutMinutes }; }
...
const handlePickChatFiles = async () => { const picked = await getDesktopApi().filesPickForChat(); setPendingChatFiles((current) => { const seen = new Set(current.map((file) => file.grantId)); return [...current, ...picked.filter((file) => { if (seen.has(file.grantId)) { return false; } seen.add(file.grantId); return true; })]; }); };
...
const openRenameCategoryDialog = (categoryPath: string) => { const category = fileCategories.find((item) => item.path === categoryPath); setRenameCategoryDialog({ open: true, categoryPath, name: category?.name ?? categoryPath.split('/').filter(Boolean).at(-1) ?? categoryPath, }); };
...
const handleSendMessage = async (nextMessage?: string, modeOverride?: { mode: ChatMode; targetAppId?: string | null }) => { const trimmed = (nextMessage ?? chatInput).trim(); const sharedFileNames = [ ...pendingChatFiles.map((file) => file.name), ...mentionedChatFiles.map((file) => file.name), ]; const userVisibleContent = trimmed || `Archivos compartidos: ${sharedFileNames.join(', ')}`; const hasAgentProvider = codexAuthStatus.authenticated || claudeAuthStatus.authenticated || antigravityAuthStatus.authenticated; const activeChatMode = modeOverride?.mode ?? activeConversation?.mode; const activeTargetAppId = activeChatMode === 'edit_app' || activeChatMode === 'social_app_review' ? modeOverride?.targetAppId ?? activeConversation?.targetAppId ?? activeConversation?.appId : null;
...
const createdConversation: ChatConversation = { id: makeConversationId(), appId: chatScopeId, mode: activeChatMode, targetAppId: activeTargetAppId, title: summarizeConversationTitle(userVisibleContent), threadId: null, runtime: createdRuntime, createdAt: now, updatedAt: now, messages: [], };
...
? await desktopApi.filesImport({ grantIds: pendingChatFiles.map((file) => file.grantId), categoryPath: uploadCategoryPath, ...(activeTargetAppId ? { appId: activeTargetAppId } : {}), }) : []; releaseChatFileSelections(stagedFilesForCleanup); const mentionedFilesForRun = forgerFiles.filter((file) => mentionedChatFileIds.includes(file.id)); const sharedFiles: SharedFileRef[] = [ ...importedFiles.map((file) => ({ id: file.id, path: file.relativePath, name: file.name, relativePath: file.relativePath,
	...
	relativePath: file.relativePath as string, displayPath: `${FORGER_DATA_ROOT_NAME}/${file.relativePath as string}`, sizeBytes: file.sizeBytes ?? 0, source: file.source as 'attached' | 'mentioned', })); const refreshedFiles = await desktopApi.filesList(fileFilters); setForgerFiles(refreshedFiles); setChatDraft(targetConversationId as string, ''); setPendingChatFiles([]); setMentionedChatFileIds([]); const now = new Date().toISOString(); setChatConversations((currentConversations) => currentConversations.map((conversation) => {
if (conversation.id !== targetConversationId) { return conversation; }
	const nextTitle = (conversation.title === 'Conversacion nueva' || conversation.title === t.sections.chat.newConversationTitle) && conversation.messages.length === 0 ? summarizeConversationTitle(userVisibleContent, t.sections.chat.newConversationTitle) : conversation.title; return { ...conversation, title: nextTitle, updatedAt: now, runtime: conversation.runtime ?? { provider: runtimeDraft.provider, model: runtimeDraft.model,
	effort: runtimeDraft.effort, ...(runtimeDraft.authProfileId ? { authProfileId: runtimeDraft.authProfileId } : {}) }, messages: [ ...conversation.messages, { id: `user-${Date.now()}`, role: 'user', content: userVisibleContent, files: messageFiles, }, ], };
...
const handleRespondQuestion = async ( runId: string, request: ChatQuestionRequest, response: ChatQuestionResponse, ) => { const targetConversationId = request.chatId; const conversationForRun = chatConversationsRef.current.find((conversation) => conversation.id === targetConversationId); if (!conversationForRun) { setBannerSeverity('error'); setBannerMessage(t.sections.chat.sendFailed('conversation_not_found')); return; }
...
const handleStartNewConversation = () => { const chatScopeId = FREE_CHAT_APP_ID; const now = new Date().toISOString(); const nextConversation: ChatConversation = { id: makeConversationId(), appId: chatScopeId, mode: 'free_chat', targetAppId: null, title: t.sections.chat.newConversationTitle, threadId: null, createdAt: now, updatedAt: now, messages: [], };
...
const handleSaveAutomation = async (input: AutomationUpsertInput & { id?: string }) => { setAutomationBusy(true); try { const desktopApi = getDesktopApi(); const saved = input.id ? await desktopApi.automationsUpdate({ ...input, id: input.id }) : await desktopApi.automationsCreate(input).then((created) => { usageAnalytics.automationCreated({ surface: 'automations', locale: t.locale }); return created; }); const nextAutomations = await refreshAutomations(); setSelectedAutomationId(saved.id); await loadAutomationRuns(saved.id); setBannerSeverity('success');
...
const nextStatus = result.success ? await pollClaudeAuthStatus(() => !isAuthConnectAttemptActive(attempt)) : result.status ?? await refreshClaudeAuthStatus(); if (isAuthConnectAttemptActive(attempt)) {
...
requestErrorReportFromResult('agent', 'claude-connect', result); } else { return; } } catch (error) { connectSettled = true; await polling; if (!isAuthConnectAttemptActive(attempt)) { return; }
...
if (result.success) { void refreshForumParticipation(); }
...
const updateForumParticipationAction = async (action: 'mark_prompt_shown' | 'opt_in') => { if (!forgerAccount.authenticated || forumParticipationBusy) { if (!forgerAccount.authenticated) { setCloudModalOpen(true); }
return; }
setForumParticipationBusy(true); try { const state = await getDesktopApi().updateForumParticipation(action); setForumParticipation(state); setForumPromptOpen(false);
if (action === 'opt_in') { setBannerSeverity('success'); setBannerMessage(t.settings.forumOptInSuccess); setCurrentView('friends'); }
...
const handleDismissForumPrompt = async () => updateForumParticipationAction('mark_prompt_shown');
const handleEnterForum = async () => updateForumParticipationAction('opt_in');
