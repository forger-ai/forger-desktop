...
const getCategoryLabel = (category: AppCategory) => t.appCategories[category]; const resetIdleChatProgress = () => { if (!chatRunActive) { setChatProgressLines([]); deliveredRunRepliesRef.current.clear(); } }; const setChatContext = (appId: string) => { const appEntry = installedApps.find((app) => app.id === appId) ?? catalogApps.find((app) => app.id === appId); if (!appEntry) { return; }
