...
open: boolean; report: ConversationDiagnosticReportPreview | null; busy: boolean; userMessage?: string; }
...
const [errorReportDialog, setErrorReportDialog] = useState<ErrorReportDialogState>({ open: false, report: null, busy: false, }); const [conversationDiagnosticDialog, setConversationDiagnosticDialog] = useState<ConversationDiagnosticDialogState>({ open: false, report: null, busy: false, }); const [selectedAppDetailsId, setSelectedAppDetailsId] = useState<string | null>(null); const [selectedAppDetails, setSelectedAppDetails] = useState<AppDetails | null>(null); const [appDetailsBackView, setAppDetailsBackView] = useState<View>('catalog'); const [selectedAppId, setSelectedAppId] = useState<string | null>(null);
...
if (link.kind === 'social-app') { setCurrentView('friends'); setBannerSeverity('info'); setBannerMessage(link.code ? 'Abriendo invitacion de Social. Revisa la app antes de instalarla.' : 'Abriendo app de Social.'); if (link.code) { void desktopApi.resolveSocialCode(link.code).catch(() => { setBannerSeverity('error'); setBannerMessage('No pudimos abrir esta invitacion de Social.'); }); } return; }
if (link.kind === 'social-profile') { setCurrentView('friends'); setBannerSeverity('info'); setBannerMessage(link.username ? `Abriendo perfil Social de @${link.username}.` : 'Abriendo Social.'); return; }
...
const clearPersistedRun = (runId?: string) => { setPersistedActiveChatRun((current) => (!runId || current?.runId === runId ? null : current)); };
const clearActiveRunState = (runId?: string) => { if (!runId || activeChatRunIdRef.current === runId) { setChatRunActive(false); activeChatRunIdRef.current = null; setActiveChatRunId(null); setChatProgressLines([]); updateActiveRunConversationId(null); }
if (runId) { runConversationIdByRunRef.current.delete(runId); }
clearPersistedRun(runId); };
const applyChatRunUpdate = (run: ChatRun) => { const currentActiveConversationId = activeConversationIdRef.current; const runConversationId = typeof run.conversationId === 'string' && run.conversationId.trim().length > 0 ? run.conversationId : null; traceChatEvent({ event: 'chat_run_event_received', runId: run.runId, appId: run.appId, conversationId: runConversationId, activeConversationId: currentActiveConversationId, status: run.status, messageCount: run.progressLog?.length ?? 0 }); const isTerminal = isTerminalChatRunStatus(run.status); if (runConversationId) { runConversationIdByRunRef.current.set(run.runId, runConversationId); if (!isTerminal) { updateActiveRunConversationId(runConversationId); setActiveConversationByApp((current) => current[run.appId] === runConversationId ? current : { ...current, [run.appId]: runConversationId }); if ((selectedAppIdRef.current ?? FREE_CHAT_APP_ID) === run.appId && (!currentActiveConversationId || currentActiveConversationId === runConversationId)) { setActiveConversationId(runConversationId); } setChatConversations((currentConversations) => { if (currentConversations.some((conversation) => conversation.id === runConversationId)) { return currentConversations; } const now = new Date().toISOString(); return [{ id: runConversationId, appId: run.appId, title: getAppMeta(run.appId).name, threadId: typeof run.threadId === 'string' && run.threadId.trim().length > 0 ? run.threadId : null, createdAt: run.createdAt ?? now, updatedAt: run.updatedAt ?? now, messages: [], }, ...currentConversations]; }); } }
if (isTerminal) { setChatRunActive(false); if (activeChatRunIdRef.current === run.runId) { activeChatRunIdRef.current = null; setActiveChatRunId(null); } clearPersistedRun(run.runId); } else { setChatRunActive(true); activeChatRunIdRef.current = run.runId; setActiveChatRunId(run.runId); const activeRun = activeRunFromChatRun(run); if (activeRun) { setPersistedActiveChatRun(activeRun); } }
...
if (!deliveredRunRepliesRef.current.has(dedupePermissionKey)) { deliveredRunRepliesRef.current.add(dedupePermissionKey); const targetConversationId = runConversationId ?? runConversationIdByRunRef.current.get(run.runId) ?? activeRunConversationIdRef.current ?? activeConversationIdRef.current; if (targetConversationId) { const permissionRequest = run.permissionRequest; const messageId = `assistant-permission-${run.runId}-${permissionRequest.requestId}`; console.info('[Forger permission] rendering request', { runId: run.runId, requestId: permissionRequest.requestId, permission: permissionRequest.permission,
resource: permissionRequest.resource, targetConversationId, }); setChatConversations((currentConversations) => currentConversations.map((conversation) => { if (conversation.id !== targetConversationId || conversation.messages.some((message) => message.id === messageId)) { return conversation; }
return { ...conversation, updatedAt: new Date().toISOString(), messages: [ ...conversation.messages, { id: messageId, role: 'assistant', content: t.sections.chat.permissionPrompt(permissionRequest.resource), action: {
type: 'permission',
...
return; }
const isMessageTerminal = isMessageTerminalChatRunStatus(run.status); if (isTerminal && !isMessageTerminal) { updateActiveRunConversationId(null); }
const hasMessage = typeof run.userMessage === 'string' && run.userMessage.trim().length > 0; const dedupeKey = `${run.runId}:${run.status}`; const messageId = `assistant-run-${run.runId}-${run.status}`; if (!isMessageTerminal || !hasMessage || deliveredRunRepliesRef.current.has(dedupeKey)) { if (isTerminal) { runConversationIdByRunRef.current.delete(run.runId); updateActiveRunConversationId(null); }
return; }
...
setChatConversations((currentConversations) => currentConversations.map((conversation) => { if (conversation.id !== targetConversationId || conversation.messages.some((message) => message.id === messageId)) { return conversation; }
return { ...conversation, threadId: typeof run.threadId === 'string' && run.threadId.trim().length > 0 ? run.threadId : conversation.threadId, updatedAt: new Date().toISOString(), messages: [ ...conversation.messages, { id: messageId, role: 'assistant', content: run.userMessage as string, }, ], };
...
setSocialUploadDialog((current) => ({ ...current, open: false })); await uploadSocialApp(appId, visibility);
};
...
if (activeRunConversationIdRef.current === conversationId) { clearActiveRunState(); }
};
...
setConversationDiagnosticDialog({ open: false, report: null, busy: false }); };
const prepareConversationDiagnosticReport = async () => { if (!activeConversation || conversationDiagnosticDialog.busy) { return; }
setConversationDiagnosticDialog({ open: true, report: null, busy: true }); try { const report = await getDesktopApi().prepareConversationDiagnosticReport({ source: 'desktop_chat', appId: activeConversation.appId === FREE_CHAT_APP_ID ? undefined : activeConversation.appId, conversationId: activeConversation.id, runId: activeConversationRunId ?? activeChatRunIdRef.current ?? undefined, title: activeConversation.title, provider: activeConversation.runtime?.provider ?? (selectedAgentProvider === 'auto' ? resolvedChatProvider : selectedAgentProvider), conversation: { appId: activeConversation.appId === FREE_CHAT_APP_ID ? undefined : activeConversation.appId, title: activeConversation.title, threadId: activeConversation.threadId, runtime: activeConversation.runtime as Record<string, unknown> | undefined, messages: activeConversation.messages.map((message) => ({ id: message.id, role: message.role, content: message.content, })), }, }); setConversationDiagnosticDialog({ open: true, report, busy: false }); } catch (error) { setConversationDiagnosticDialog({ open: true, report: null, busy: false, userMessage: error instanceof Error ? error.message : t.settings.conversationReportPrepareError }); } };
const copyConversationDiagnosticReport = async () => { if (!conversationDiagnosticDialog.report) { return; }
await navigator.clipboard.writeText(JSON.stringify(conversationDiagnosticDialog.report, null, 2)); setConversationDiagnosticDialog((current) => ({ ...current, userMessage: t.settings.conversationReportCopied })); };
const submitConversationDiagnosticReport = async () => { if (!conversationDiagnosticDialog.report) { return; }
setConversationDiagnosticDialog((current) => ({ ...current, busy: true, userMessage: undefined })); try { const result = await getDesktopApi().submitConversationDiagnosticReport(conversationDiagnosticDialog.report); if (result.success) { setBannerSeverity('success'); setBannerMessage(result.userMessage); setConversationDiagnosticDialog({ open: false, report: null, busy: false }); return; }
