...
- `app_action`: executes one exact MCP tool from one installed app without an LLM. The node stores `appId`, `toolName`, template-resolved JSON input, and a presentation/schema snapshot so a missing app or renamed action stays visible instead of being substituted. Desktop lists only tools with object `inputSchema`, object `outputSchema`, and non-task execution; successful calls must return an object in `structuredContent` that validates against the declared output schema.
...
Before a run containing app actions starts, Desktop acquires every required app MCP and verifies every exact tool and schema. No workflow node starts when that preflight fails. The same run-scoped MCP clients stay bound through execution, calls are never retried automatically, and schema or risk drift stops the run before the changed action executes. Destructive, external, or unknown effects always require approval. MCP tokens, endpoints, and raw metadata stay in the main process and never cross the renderer bridge.
