...
test('given Antigravity reinstall or disconnect failures, the public APIs return safe diagnostics', async () => {
  const reinstall = await makeHarness({
    runCommand: async () => { throw new Error('managed installer failed'); },
  });
  try {
    const result = await withPlatform('darwin', () => reinstall.controller.reinstallAntigravity());
    assert.equal(result.success, false);
    assert.equal(result.technicalCode, 'managed installer failed');
  } finally {
    await reinstall.cleanup();
  }

  const disconnect = await makeHarness({
    markProviderDisconnected: async () => { throw new Error('disconnect failed'); },
  });
  try {
    const result = await disconnect.controller.disconnectAntigravityAuth();
    assert.equal(result.success, false);
    assert.equal(result.technicalCode, 'disconnect failed');
  } finally {
    await disconnect.cleanup();
  }
});
...
test('given managed Antigravity install invariants, existing, post-reset, and unsupported platforms are explicit', async () => {
  ...
  const resetFs = Object.assign(Object.create(fsWithAvailableDisk), {
    rm: async (target, options) => {
