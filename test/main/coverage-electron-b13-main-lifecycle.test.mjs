...
const createLifecycleHarness = async (overrides = {}, harnessOptions = {}) => {
  ...
  const richService = (name, options = {}) => ({
    ...
    review: async (workflowId) => ({ workflowId, status: 'ready' }),
    ...
    apply: async (workflowId, input) => ({ workflowId, input }),
  ...
  const explicit = {
    ...
    WorkflowAppActionRuntime: serviceClass('WorkflowAppActionRuntime'),
...
test('given a ready desktop, lifecycle options compose cloud, tools, agents, audio, and workflow boundaries', async () => {
  ...
  try {
    ...
    assert.deepEqual(await mcp.workflowsReview('workflow'), { workflowId: 'workflow', status: 'ready' });
    assert.deepEqual(await mcp.workflowsApply('workflow', { expectedRevision: 1 }), {
      workflowId: 'workflow',
    ...
    await assert.rejects(mcp.workflowsReview('workflow'), /workflow_manager_unavailable/);
    await assert.rejects(mcp.workflowsApply('workflow', {}), /workflow_manager_unavailable/);
