...
test('BackupsManager reports batch cardinality and successful cloud restore observably', async (t) => {
  ...
  const originalCreateBackupUnlocked = manager.createBackupUnlocked;
  manager.createBackupUnlocked = async () => ({
    success: false,
    userMessage: 'No se pudo crear el respaldo previo.',
    technicalCode: 'pre_restore_failed',
  });
  assert.deepEqual(await manager.restoreBackupDirectory({ appId: appRecord.appId, backupDir: cloudDir }), {
    success: false,
    userMessage: 'No se pudo crear el respaldo previo.',
    technicalCode: 'pre_restore_failed',
  });
  manager.createBackupUnlocked = originalCreateBackupUnlocked;
