...
test('given workflow bridge gaps, node and management tools return stable unavailable diagnostics', async () => {
  ...
  assert.equal((await call(server, workflow, 'forger_workflow_list')).technicalCode, 'workflow_management_not_allowed');
  assert.deepEqual((await call(server, desktop, 'forger_workflow_list')).workflows, []);
  assert.equal((await call(server, desktop, 'forger_workflow_upsert')).technicalCode, 'workflow_manager_unavailable');
  assert.equal((await call(server, desktop, 'forger_workflow_run')).technicalCode, 'workflow_manager_unavailable');

  server.options.workflowsRun = async () => { throw 'workflow-offline'; };
  assert.equal((await call(server, desktop, 'forger_workflow_run', { workflowId: 'flow' })).technicalCode, 'workflow_operation_failed');
});
