...
const data: WorkflowGraphData = {
  apps: [], agents: [], toolPackages: [], officialTools: [], connectionOptions: [], providerOptions: [], appActions: [],
...
const workflow = (overrides: Partial<Workflow> = {}): Workflow => ({
  ...
  revision: 1, appliedRevision: 1, ...overrides,
});
...
const renderDetail = ({
  ...
  revisions = [], review = null, reviewOpen = false,
}: HarnessProps = {}) => {
  const callbacks = {
    ...
    onReloadRevisions: vi.fn(), onRestoreRevision: vi.fn(), onReview: vi.fn(), onCloseReview: vi.fn(), onApplyReview: vi.fn(),
    onRetryRun: vi.fn(),
  };
  const Harness = () => {
    ...
    return (
      <>
        <WorkflowDetailPage
          ...
          revisions={revisions} onReloadRevisions={callbacks.onReloadRevisions} onRestoreRevision={callbacks.onRestoreRevision}
          onReview={callbacks.onReview} review={review} reviewOpen={reviewOpen} onCloseReview={callbacks.onCloseReview} onApplyReview={callbacks.onApplyReview}
          runs={runs} selectedRunId={selectedRunId} selectedRun={selectedRun} onRetryRun={callbacks.onRetryRun}
        />
...
describe('WorkflowDetailPage', () => {
  it('handles idle editing, dirty actions, banners, and parameter rollback/save', async () => {
    ...
    await user.click(screen.getByRole('button', { name: copy.deactivateSchedule }));
    await user.click(screen.getByRole('button', { name: copy.runApplied }));
  ...
  it('toggles empty history and lists every run status and trigger', async () => {
    ...
    const callbacks = renderDetail({ workflowValue: workflow({ enabled: false, trigger: { type: 'scheduled', frequency: { type: 'daily', time: '09:00' } }, appliedTrigger: { type: 'scheduled', frequency: { type: 'daily', time: '09:00' } } }), runs, selectedRunId: 'run-succeeded' });
  ...
  it('shows empty history and closes parameter edits through Escape', async () => {
    ...
    const historyPanel = screen.getByText(copy.noRuns).closest('.MuiPaper-root') as HTMLElement;
  ...
  it('covers review, revision restore, pending approval, and failed-run recovery flows', async () => {
    const user = userEvent.setup();
    const pending = detailedRun('waiting_approval');
    pending.pendingApprovalNodeId = 'node-1';
    const revisions: WorkflowRevisionSummary[] = [
      { id: 'revision-1', workflowId: 'workflow-1', revision: 1, definitionHash: 'hash-1', createdAt: '2026-08-10T10:00:00.000Z', applied: true },
      { id: 'revision-2', workflowId: 'workflow-1', revision: 2, definitionHash: 'hash-2', createdAt: '2026-08-11T10:00:00.000Z', applied: false },
    ];
    const callbacks = renderDetail({
      workflowValue: workflow({ revision: 2, appliedRevision: 1, trigger: { type: 'scheduled', frequency: { type: 'daily', time: '09:00' } }, appliedTrigger: { type: 'scheduled', frequency: { type: 'daily', time: '09:00' } } }),
      selectedRun: pending,
      selectedRunId: 'run-other',
      runs: [pending],
      revisions,
      review: { status: 'ready', issues: [], definitionHash: 'hash-1' },
      reviewOpen: true,
    });

    await user.click(screen.getByText(copy.pendingApproval));
    expect(callbacks.onSelectRun).toHaveBeenCalledWith('run-waiting_approval');
    await user.click(screen.getByRole('button', { name: copy.applyReview }));
    expect(callbacks.onApplyReview).toHaveBeenCalledOnce();
    await user.click(screen.getByRole('button', { name: copy.cancel }));
    expect(callbacks.onCloseReview).toHaveBeenCalledOnce();

    cleanup();
    const revisionCallbacks = renderDetail({ revisions });
    await user.click(screen.getByRole('button', { name: copy.revisions }));
    expect(revisionCallbacks.onReloadRevisions).toHaveBeenCalledOnce();
    const restoreButtons = screen.getAllByRole('button', { name: copy.restoreAsDraft });
    await user.click(restoreButtons[0]);
    const restoreDialog = screen.getByRole('dialog', { name: copy.restoreRevisionTitle });
    await user.click(within(restoreDialog).getByRole('button', { name: copy.cancel }));
    await user.click(screen.getAllByRole('button', { name: copy.restoreAsDraft })[1]);
    expect(screen.getByText(copy.restoreRevisionWarning)).toBeVisible();
    const confirmRestore = screen.getAllByRole('button', { name: copy.restoreAsDraft }).at(-1);
    await user.click(confirmRestore!);
    expect(revisionCallbacks.onRestoreRevision).toHaveBeenCalledWith(revisions[1]);

    cleanup();
    renderDetail({ revisions });
    await user.click(screen.getByRole('button', { name: copy.revisions }));
    const revisionsDialog = screen.getByRole('dialog', { name: copy.revisions });
    await user.click(within(revisionsDialog).getByRole('button', { name: copy.cancel }));

    cleanup();
    const safeFailed = detailedRun('failed');
    safeFailed.safeToRetry = true;
    const rerendered = renderDetail({ selectedRun: safeFailed, selectedRunId: safeFailed.id });
    await user.click(screen.getByRole('button', { name: copy.retryRun }));
    expect(rerendered.onRetryRun).toHaveBeenCalledWith('run-failed');

    cleanup();
    const unsafeFailed = detailedRun('failed');
    unsafeFailed.safeToRetry = false;
    const unsafeCallbacks = renderDetail({ selectedRun: unsafeFailed, selectedRunId: unsafeFailed.id });
    await user.click(screen.getByRole('button', { name: copy.startNewRun }));
    await user.click(screen.getByRole('button', { name: copy.cancel }));
    await user.click(screen.getByRole('button', { name: copy.startNewRun }));
    const startButtons = screen.getAllByRole('button', { name: copy.startNewRun });
    await user.click(startButtons.at(-1)!);
    expect(unsafeCallbacks.onRunNow).toHaveBeenCalled();
  });

  it('covers draft status, an already-selected approval, and the busy restore lock', async () => {
    const user = userEvent.setup();
    renderDetail({ workflowValue: workflow({ enabled: false, revision: 1, appliedRevision: 0, appliedTrigger: undefined }) });
    expect(screen.getByText(copy.draftRevision)).toBeVisible();

    cleanup();
    renderDetail({ workflowValue: workflow({ enabled: false, revision: 2, appliedRevision: 1, appliedTrigger: undefined }) });
    expect(screen.getByText(copy.appliedRevision)).toBeVisible();

    cleanup();
    const selected = detailedRun('waiting_approval');
    selected.pendingApprovalNodeId = 'node-1';
    const sameSelection = renderDetail({ selectedRun: selected, selectedRunId: selected.id, runs: [selected] });
    await user.click(screen.getByText(copy.pendingApproval));
    expect(sameSelection.onSelectRun).not.toHaveBeenCalled();

    cleanup();
    const revisions: WorkflowRevisionSummary[] = [{
      id: 'revision-1', workflowId: 'workflow-1', revision: 1,
      definitionHash: 'hash-1', createdAt: '2026-08-10T10:00:00.000Z', applied: false,
    }];
    const callbacks = {
      onClearBanner: vi.fn(), onSave: vi.fn(), onDiscard: vi.fn(), onBack: vi.fn(), onRunNow: vi.fn(),
      onToggleEnabled: vi.fn(), onRunNode: vi.fn(), onSelectRun: vi.fn(), onApproveNode: vi.fn(), onCancelRun: vi.fn(),
    ...
    const Harness = () => {
      ...
      return (
        <>
          <button onClick={() => setBusy(true)}>Make busy</button>
          <WorkflowDetailPage
            t={t} workflow={workflow()} draft={{ ...emptyDraft(), name: 'Draft' }} onDraftChange={vi.fn()} data={data}
            dirty={false} busy={busy} banner={null} {...callbacks} revisions={revisions}
            runs={[]} selectedRunId={null} selectedRun={null}
          />
        </>
      );
    };
    render(<Harness />);
    await user.click(screen.getByRole('button', { name: copy.revisions }));
    await user.click(screen.getByRole('button', { name: copy.restoreAsDraft }));
