...
describe('RendererAppController settings, backup, automation, and file action variants', () => {
  ...
  it('covers empty, skipped, canceled, failed, and replacement automation behavior', async () => {
    ...
    await act(async () => result.current.handleSaveAutomation({ id: 'automation-1', name: 'Daily', enabled: false, selectedAppIds: [], prompt: '', frequency: 'daily' } as never));
    expect(bridge.call('automationsUpdate')).toHaveBeenCalledWith(expect.objectContaining({ id: 'automation-1' }));
    bridge.set('automationsUpdate', { ...automation, enabled: true });
    await act(async () => result.current.handleSaveAutomation({ id: 'automation-1', name: 'Daily', enabled: true, selectedAppIds: [], prompt: '', frequency: 'daily' } as never));
