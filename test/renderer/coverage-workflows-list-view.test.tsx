...
describe('WorkflowsListView', () => {
  ...
  it('renders running, active, paused, manual, scheduled, and timestamp states', async () => {
    ...
    const draft = workflow({
      ...
      revision: 1,
      appliedRevision: 0,
      appliedTrigger: undefined,
    });
    const appliedManual = workflow({
      id: 'applied-manual', name: 'Applied manual', description: undefined,
      enabled: false, revision: 2, appliedRevision: 1, appliedTrigger: undefined,
    });
    const callbacks = renderList([running, paused, active, draft, appliedManual]);
    ...
    expect(screen.getAllByText(copy.triggerManual)).toHaveLength(4);
  ...
  it('runs a workflow and toggles both enabled states while respecting running and global locks', async () => {
    ...
    const disabled = workflow({
      ...
      name: 'Disabled flow',
      enabled: false,
      trigger: { type: 'scheduled', frequency: { type: 'daily', time: '09:00' } },
      appliedTrigger: { type: 'scheduled', frequency: { type: 'daily', time: '09:00' } },
    });
