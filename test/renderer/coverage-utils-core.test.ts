...
describe('renderer core utility behavior', () => {
  ...
  it('round-trips workflow drafts and creates every supported node kind', () => {
    ...
    expect(draftFromWorkflow({ ...workflow, revision: 4 })).toHaveProperty('revision', 4);
    expect(draftToUpsertInput({ ...draft, revision: 4 })).toHaveProperty('expectedRevision', 4);
    ...
    expect(nextNodeId([{ ...nodes[0], id: 'paso1' }, { ...nodes[0], id: 'paso2' }], 'paso')).toBe('paso3');
