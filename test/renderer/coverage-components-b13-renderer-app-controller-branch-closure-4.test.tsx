...
describe('RendererAppController late branch and invariant closure', () => {
  ...
  it('selects the first available run and clears selection when an automation history becomes empty', async () => {
    let runs: ReturnType<typeof transcript>[] = [];
    const bridge = installControllerBridge({
      automationsList: [automation],
  ...
  it('uses localized fallbacks for non-Error create, update, chat, stop, and automation failures', async () => {
    ...
    const bridge = installControllerBridge({
      ...
      automationsCreate: { id: 'created-automation', name: 'Created', enabled: true, selectedAppIds: [], createdAt: '2026-08-10', updatedAt: '2026-08-10' },
      automationsList: [{ id: 'created-automation', name: 'Created', enabled: true, selectedAppIds: [], createdAt: '2026-08-10', updatedAt: '2026-08-10' }],
      automationsListRuns: [],
      filesList: [],
    });
    const { result } = await renderControllerHarness(bridge);
    ...
    await act(async () => result.current.handleSaveAutomation({ name: 'Created', enabled: true, selectedAppIds: [] } as never));
    bridge.set('automationsUpdate', { id: 'updated-automation', name: 'Updated', enabled: true, selectedAppIds: [], createdAt: '2026-08-10', updatedAt: '2026-08-10' });
    await act(async () => result.current.handleSaveAutomation({ id: 'updated-automation', name: 'Updated', enabled: true, selectedAppIds: [] } as never));
